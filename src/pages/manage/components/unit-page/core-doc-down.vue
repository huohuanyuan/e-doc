 ...
 * @Last Modified time: 2018-11-19 16:16:51
 */
<template>
...
export default {
  ...
  methods: {
    ...
    getAllTree() {
      this.spinShow = true;
      ApiGetLibraryFolder({ central: true }).then(res => {
        this.spinShow = false;
        if (res.code == "success") {
    ...
    getFileList() {
      ApiGetLibraryFileList(this.fileListParams).then(res => {
        if (res.code == "success") {
